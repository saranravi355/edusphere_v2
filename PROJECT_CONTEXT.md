...
- **Sign-in is in DEMO MODE.** Every account opens with `password123`, the login form pre-fills it, and nobody is forced to change it — deliberate, so the system can be shown without handing out credentials. The machinery to close it exists and is switched off by one variable: set `NEXT_PUBLIC_FORCE_PASSWORD_RESET=true` (Vercel env, then redeploy) and new accounts get a single-use password each, the login form stops pre-filling, and `/change-password` becomes compulsory. To also re-flag the accounts that already exist, run `UPDATE "User" SET "mustChangePassword" = true WHERE "passwordChangedAt" IS NULL;`. See `src/lib/demo.ts`. **Do this before real families are in the database.**
- What was fixed for good on 28 Aug and is NOT part of the switch: the `@default("password123")` is gone from `User.password`, so a row created without one now fails; session cookies no longer carry the password hash (they carry id, email, name, role and the flag, with `SESSION_VERSION` invalidating older ones); `/change-password` exists and works voluntarily; `AuditLog` records sign-ins, failed sign-ins, password changes and account creation.
- Still missing whenever you do go live: 2FA, rate limiting on sign-in, and forgotten-password self-service (needs an email provider).
