...
/**
 * Only the transport manager and the administrators get in here. Another
 ...
 */
export default async function TransportOperationsLayout({ children }: { children: React.ReactNode }) {
  const session = await getSession();
  const role: string | undefined = session?.user?.role;
  if (!canAccessDepartment(role, "transport")) redirect(operationsLandingPath(role));
  return <>{children}</>;
}
