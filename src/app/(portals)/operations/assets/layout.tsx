...
/**
 * Only the assets manager and the administrators get in here. Another
 ...
 */
export default async function AssetsOperationsLayout({ children }: { children: React.ReactNode }) {
  const session = await getSession();
  const role: string | undefined = session?.user?.role;
  if (!canAccessDepartment(role, "assets")) redirect(operationsLandingPath(role));
  return <>{children}</>;
}
