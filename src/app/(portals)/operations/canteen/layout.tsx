...
/**
 * Only the canteen manager and the administrators get in here. Another
 ...
 */
export default async function CanteenOperationsLayout({ children }: { children: React.ReactNode }) {
  const session = await getSession();
  const role: string | undefined = session?.user?.role;
  if (!canAccessDepartment(role, "canteen")) redirect(operationsLandingPath(role));
  return <>{children}</>;
}
