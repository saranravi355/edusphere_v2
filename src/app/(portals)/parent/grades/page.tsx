  ...
  return (
    <div className="space-y-6 pb-12 max-w-5xl">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        {/*
          This was a <button> in a Server Component with no onClick, no form and
          no type — the one control on the page, and it did nothing. It reuses
          the same print-to-PDF the student report card already uses.
        */}
        <PrintButton targetId="grades-print" label="Download as PDF" className="flex items-center gap-2 px-4 py-2 bg-slate-800 hover:bg-slate-900 text-white dark:bg-white dark:text-slate-900 dark:hover:bg-slate-100 rounded-lg text-sm font-medium transition-colors shadow-sm" />
      </div>

      <div id="grades-print" className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm overflow-hidden">
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
              {grades.map(grade => (
                <tr key={grade.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/20 transition-colors">
                  ...
                  <td className="px-6 py-4 text-slate-600 dark:text-slate-400">{formatDate(grade.date, "ddmmyyyy")}</td>
