...
export default async function ParentMessagesPage({
  ...
  // The parent's default contacts: each child's class teacher.
  ...
  // Build a contact list keyed by the other user's id: existing conversation
  // partners first, plus each child's class teacher so a parent can always
  ...
  for (const s of parent?.students ?? []) {
    const t = s.classroom?.teacher?.user;
    if (t && !contacts.has(t.id)) {
      contacts.set(t.id, {
        id: t.id,
        name: t.name,
        subtitle: `Class teacher · ${s.classroom?.name ?? ""}`.trim(),
        lastAt: 0,
      });
    }
  }
  ...
  return (
    <div className="h-[calc(100vh-8rem)] flex flex-col max-w-6xl">
      <PageHeader
        title="Messages"
        description="Communicate with your child's teachers and the school office."
      />
              ...
              <div className="divide-y divide-slate-100 dark:divide-slate-800/50">
                {contactList.map((c) => {
                  ...
                  return (
                    <Link
                      key={c.id}
                      href={`/parent/messages?with=${c.id}`}
        ...
        <div className="flex-1 flex flex-col bg-white dark:bg-slate-900">
          {selected ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-slate-500">
              ...
              <p>No teachers available to message yet.</p>
            </div>
          )}
        </div>
      </div>
